...
/**
	 ...
	 */const d={name:"Smiles",components:{LoadingTab:r,SmilesSetTab:a,EmojiTab:n},directives:{lazyload:m.lazyload},props:{isOnlyEmoji:{type:Boolean,default:false}},emits:["selectSmile","selectSet"],data(){return{smiles:[],sets:[],setSelected:0,mode:"smile",emojiIcon:"\ud83d\ude0d"}},computed:{isShowEmoji(){return this.$Bitrix.Loc.getMessage("UTF_MODE")==="Y"},isShowSmiles(){return!this.isOnlyEmoji},isLoading(){return this.isShowSmiles&&this.smiles.length<=0},isEmojiMode(){return this.mode==="emoji"},isSmileMode(){return this.mode==="smile"},isShowTabsSelector(){if(this.isShowSmiles&&this.sets.length>1){return true}return this.isShowSmiles&&this.isShowEmoji},emojiIconStyle(){const s="bx-ui-smiles-set-emoji";if(t()){return`${s}-mac`}if(e()){return`${s}-linux`}if(i()){return`${s}-win`}return s}},created(){if(this.isShowSmiles){this.mode="smile"}else if(this.isShowEmoji){this.mode="emoji"}if(this.isShowSmiles){this.smilesController=new b(this.$Bitrix.RestClient.get());void this.smilesController.loadFromCache().then((s=>{this.smiles=s.smiles;this.sets=s.sets.map(((s,l)=>{s.selected=this.setSelected===l;return s}))}));void this.smilesController.loadFromServer().then((s=>{this.smiles=s.smiles;this.sets=s.sets.map(((s,l)=>{s.selected=this.setSelected===l;return s}))}))}},methods:{selectSet(s){if(!this.isShowSmiles){return}this.mode="smile";this.$emit("selectSet",{setId:s});void this.smilesController.changeSet(s).then((l=>{this.smiles=l;this.sets.map((l=>{l.selected=l.id===s;if(l.selected){this.setSelected=s}return l}));this.$refs.elements.scrollTop=0}))},selectSmile(s){this.$emit("selectSmile",{text:` ${s.text} `})},switchToEmoji(){if(!this.isShowEmoji){return}this.mode="emoji";this.sets.forEach((s=>{s.selected=false}))}},template:`\n\t\t<div class="bx-ui-smiles-box">\n\t\t\t<div class="bx-ui-smiles-elements-wrap" ref="elements">\n\t\t\t\t<LoadingTab v-if="isLoading"/>\n\t\t\t\t<SmilesSetTab v-else-if="isSmileMode" :smiles="smiles" @selectSmile="selectSmile"/>\n\t\t\t\t<EmojiTab v-else-if="isEmojiMode" @selectSmile="selectSmile"/>\n\t\t\t</div>\n\t\t\t<template v-if="isShowTabsSelector">\n\t\t\t\t<div class="bx-ui-smiles-sets">\n\t\t\t\t\t<template v-if="isShowSmiles">\n\t\t\t\t\t\t<template v-for="set in sets">\n\t\t\t\t\t\t\t<div :class="['bx-ui-smiles-set', {'bx-ui-smiles-set-selected': set.selected}]">\n\t\t\t\t\t\t\t\t<img v-lazyload\n\t\t\t\t\t\t\t\t\t :key="set.id"\n\t\t\t\t\t\t\t\t\t class="bx-ui-smiles-set-icon"\n\t\t\t\t\t\t\t\t\t :data-lazyload-src="set.image"\n\t\t\t\t\t\t\t\t\t data-lazyload-error-class="bx-ui-smiles-set-icon-error"\n\t\t\t\t\t\t\t\t\t :title="set.name"\n\t\t\t\t\t\t\t\t\t @click="selectSet(set.id)"\n\t\t\t\t\t\t\t\t/>\n\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t</template>\n\t\t\t\t\t</template>\n\t\t\t\t\t<div v-if="isShowEmoji" :class="[\n\t\t\t\t\t\t'bx-ui-smiles-set',\n\t\t\t\t\t\t{\n\t\t\t\t\t\t\t'bx-ui-smiles-set-selected': isEmojiMode,\n\t\t\t\t\t\t},\n\t\t\t\t\t]">\n\t\t\t\t\t\t<div :class="['bx-ui-smiles-set-icon', emojiIconStyle]" @click="switchToEmoji">\n\t\t\t\t\t\t\t{{ emojiIcon }}\n\t\t\t\t\t\t</div>\n\t\t\t\t\t</div>\n\t\t\t\t</div>\n\t\t\t</template>\n\t\t</div>\n\t`};s.Smiles=d;s.emoji=y})(this.BX.Vue3.Components=this.BX.Vue3.Components||{},BX,BX.DexieExport,BX.Vue3.Directives);
