...
(function (exports,main_core,main_core_events,sale_checkout_provider_rest,sale_checkout_const,sale_checkout_lib) {
    ...
    var Application = /*#__PURE__*/function () {
      ...
      babelHelpers.createClass(Application, [{
        ...
        key: "isAllowOrderSave",
        value: function isAllowOrderSave() {
        ...
        key: "disallowOrderSave",
        value: function disallowOrderSave() {
          this.orderSaveAllowed = false;
        }
        /**
         * @private
        ...
        value: function subscribeToEvents() {
          ...
          main_core_events.EventEmitter.subscribe(sale_checkout_const.EventType.consent.refused, function (e) {
            return _this2.handlerConsentRefused(e);
          });
          main_core_events.EventEmitter.subscribe(sale_checkout_const.EventType.consent.accepted, function (e) {
            return _this2.handlerConsentAccepted(e);
          });
        ...
        value: function handlerConsentAccepted(event) {
          var item = event.getData()[0];
          this.store.dispatch('consent/setChecked', {
            id: item.config.id,
            checked: 'Y'
          });
        ...
        value: function handlerConsentRefused(event) {
          var item = event.getData()[0];
          this.store.dispatch('consent/setChecked', {
            id: item.config.id,
            checked: 'N'
        ...
        value: function sendEventsForCheckedConsents() {
          ...
          consent.items.forEach(function (item) {
            if (item.checked === 'Y') {
              main_core_events.EventEmitter.emit("".concat(sale_checkout_const.Consent.validate.submit, "-").concat(item.id), []);
            }
          });
        }
      }, {
        key: "getFirstRequiredUncheckedConsentItem",
        value: function getFirstRequiredUncheckedConsentItem() {
          var consent = this.store.getters['consent/get'];
          return consent.items.find(function (item) {
        ...
        value: function handlerCheckout() {
          ...
          var firstRequiredUncheckedConsentItem = this.getFirstRequiredUncheckedConsentItem();
          if (firstRequiredUncheckedConsentItem) {
            main_core_events.EventEmitter.emit("".concat(sale_checkout_const.Consent.validate.submit, "-").concat(firstRequiredUncheckedConsentItem.id), []);
            return;
          }
