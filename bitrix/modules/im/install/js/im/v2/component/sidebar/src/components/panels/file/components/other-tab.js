...
import { FileViewerContext, SidebarDetailBlock, SidebarFileTypes } from 'im.v2.const';
...
export const OtherTab = {
	...
	computed:
	{
		...
		files(): ImModelSidebarFileItem[]
		{
			if (this.isSearch)
			{
				return this.$store.getters['sidebar/files/getSearchResultCollection'](this.chatId, SidebarFileTypes.other);
			}
	...
	methods:
	{
		...
		needToLoadNextPage(event: Event): boolean
		{
			...
			const hasNextPage = this.$store.getters[nameGetter](this.chatId, SidebarFileTypes.other);
		...
		async onScroll(event: Event)
		{
			...
			{
				await this.serviceSearch.loadNextPage(SidebarFileTypes.other, this.searchQuery);
			}
