<?php
$MESS["CP_BNL_CACHE_GROUPS"] = "Қолжетімділік құқықтарын ескеру";
$MESS["CP_BNL_INCLUDE_SUBSECTIONS"] = "Бөлімнің бөлімшелерінің элементтерін көрсету";
$MESS["CP_BNL_SET_BROWSER_TITLE"] = "Браузер терезесінің тақырыпатын орнату";
$MESS["CP_BNL_SET_META_DESCRIPTION"] = "Парақшаныі сипаттамасын орнату";
$MESS["CP_BNL_SET_META_KEYWORDS"] = "Парақшаның кілт сөздерін орнату";
$MESS["CP_BNL_SET_STATUS_404"] = "Элемент немесе бөлім табылмаса, 404 мәртебесін орнату.";
$MESS["IBLOCK_CACHE_FILTER"] = "Сүзгі орнатылған кезде кэштеу";
$MESS["IBLOCK_FIELD"] = "Өрістер";
$MESS["IBLOCK_SECTION_CODE"] = "Бөлім коды";
$MESS["IBLOCK_SECTION_ID"] = "Бөлім ID-і";
$MESS["T_IBLOCK_DESC_ACTIVE_DATE_FORMAT"] = "Күнді көрсету пішімі";
$MESS["T_IBLOCK_DESC_ADD_SECTIONS_CHAIN"] = "Бөлімді навигация тізбегіне қосу";
$MESS["T_IBLOCK_DESC_ASC"] = "Өсу бойынша";
$MESS["T_IBLOCK_DESC_CHECK_DATES"] = "Қазіргі уақытта тек белсенді элементтерді көрсету";
$MESS["T_IBLOCK_DESC_DESC"] = "Кему бойынша";
$MESS["T_IBLOCK_DESC_DETAIL_PAGE_URL"] = "Егжей-тегжейлі қарау бетінің URL (әдепкі бойынша - ақпараттық блок баптауларынан)";
$MESS["T_IBLOCK_DESC_FACT"] = "Белсенділіктің басталу күні";
$MESS["T_IBLOCK_DESC_FID"] = "ID";
$MESS["T_IBLOCK_DESC_FNAME"] = "Атауы";
$MESS["T_IBLOCK_DESC_FSORT"] = "Сұрыптау";
$MESS["T_IBLOCK_DESC_FTSAMP"] = "Соңғы өзгертілу күні";
$MESS["T_IBLOCK_DESC_HIDE_LINK_WHEN_NO_DETAIL"] = "Егжей-тегжейлі сипаттама болмаса, сілтемені жасыру";
$MESS["T_IBLOCK_DESC_IBBY1"] = "Жаңалықтарды бірінші сұрыптау үшін бағыт";
$MESS["T_IBLOCK_DESC_IBBY2"] = "Жаңалықтарды екінші сұрыптау үшін бағыт";
$MESS["T_IBLOCK_DESC_IBORD1"] = "Жаңалықтарды бірінші сұрыптау үшін өріс";
$MESS["T_IBLOCK_DESC_IBORD2"] = "Жаңалықтарды екінші сұрыптау үшін өріс";
$MESS["T_IBLOCK_DESC_INCLUDE_IBLOCK_INTO_CHAIN"] = "Ақпараттық блокты навигация тізбегіне қосу";
$MESS["T_IBLOCK_DESC_LIST_CONT"] = "Парақшадағы жаңалықтар саны";
$MESS["T_IBLOCK_DESC_LIST_ID"] = "Ақпараттық блок коды";
$MESS["T_IBLOCK_DESC_LIST_TYPE"] = "Ақпараттық блоктың түрі (тек тексеру үшін қолданылады)";
$MESS["T_IBLOCK_DESC_PAGER_NEWS"] = "Жаңалықтар";
$MESS["T_IBLOCK_DESC_PREVENT_SEND_IF_NO_NEWS"] = "Жаңа элементтердің көрсетілген саны жеткіліксіз болса, шығарылымды жібермеу.";
$MESS["T_IBLOCK_DESC_PREVIEW_TRUNCATE_LEN"] = "Шығару үшін аңдатпаның максималды ұзындығы (тек мәтін түрі үшін)";
$MESS["T_IBLOCK_DESC_SENDER_CHAIN_ID"] = "Жіберілімнің мерзімді тарату коды.";
$MESS["T_IBLOCK_FILTER"] = "Сүзгі";
$MESS["T_IBLOCK_PROPERTY"] = "Қасиеттері";
